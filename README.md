...
## Script Implemenation
The implementation of run_analysis.R uses the dplyr library along with base R functionality to manipulate the input data.

A base assumption for the script is the data contains to missing values and that the number of records in the measurements, label and subject files fit to each other.
...
Descriptive labels based on the *activity_labels.txt* are joined to each loaded data set. The subject key read from the respective files and are appended to the data sets.

### Merging the test and train data sets
Both test and train data set have the same column layout and can combined into one data frame using the *union()* function of R.

### Generating the subject activity analysis data
To generate the out put from the tidied data set dplyr in combination with pipes is used.

Together with grouping by subject key and activity the summarise_each function is used to apply the mean function to all measurements columns. A subsequent update of the column names indicate that they are mean values.
...
Each row contains the means for all features in question and a specific activity done by an individual subject.

The following table provides an list of all columns in the data file.
